...
$keyPrivate = new Key('file://{path to your private key}');
...
$token = (new Builder())->setIssuer('http://example.com') // Configures the issuer (iss claim)
                        ...
                        ->setExpiration(time() + 3600) // Configures the expiration time of the token (exp claim)
                        ->set('uid', 1) // Configures a new claim, called "uid"
                        ->sign($signer,  $keyPrivate) // creates a signature using your private key
                        ->getToken(); // Retrieves the generated token

$keyPublic = new Key('file://{path to your public key}');

var_dump($token->verify($signer, $keyPublic)); // true when the public key was generated by the private one =)
```
